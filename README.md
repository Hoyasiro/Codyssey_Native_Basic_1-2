...
| 화면 구성 | 검은색 배경에 주변에서 수근 거리는 소리가 들린다. 목소리가 텍스트로 화면 좌측 상단과 우측 하단에 1초씩 노출된다. 문자는 fade in/out으로 부드럽게 표현된다. 좌측상단 문장: "야구선수로 키우려면 공부는 포기해야 한다며?", 우측하단 문장: "애들은 가족 여행도 못간다던데..." | 투수가 와인드업 자세를 취한 뒤 공을 힘차게 던지는 모습. | 타자가 공을 힘차게 치는 모습. | 다시 검은 배경에 '대전서구이글스' 로고와 슬로건("공부하는 야구, 생활 속의 야구, 즐기는 야구") 문자가 페이드인하며 영상이 끝. |
