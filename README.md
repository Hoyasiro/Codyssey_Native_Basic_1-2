...
| 화면 구성 | 흰 배경에 검정색 말머리 풍선에 텍스트("야구선수로 키우려면 공부는 포기해야 한다며?")가 부드럽게 나타났다 사라진다.(fade in/out) | 투수가 와인드업 자세를 취한 뒤 공을 힘차게 던지는 모습. | 타자가 공을 힘차게 치는 모습. | 다시 검은 배경에 '대전서구이글스' 로고와 슬로건("공부하는 야구, 생활 속의 야구, 즐기는 야구") 문자가 페이드인하며 영상이 끝. |
...
|생성 결과 요약|Brass 사운드가 경쾌한 BGM이 생성되었다. 도입부와 최종 멘트에서 잘 어울리는 비트와 멜로디가 생성된 건 굉장히 운이 좋았던 것 같다.|
